...
import { FLORA_SECTIONS, FLORA_EXTRA_EMPTY } from '../../config/floraFields';
import { useToast } from '../../components/common/ToastContext';

const EMPTY_FORM = {
  name: '', scientific_name: '', category_id: '', family: '',
  conservation_status: 'DD', description: '', habitat: '', characteristics: '', uses: '', status: 'published',
  ...FLORA_EXTRA_EMPTY,
};

/**
 * CRUD Flore côté Membre — fonctionne désormais à l'identique de la Faune :
 * régions en liste déroulante manuelle (bouton +) ET points GPS multiples
 * (bouton Ajouter, à la main ou via la carte), les deux étant indépendants.
 * Item : barre de recherche pour retrouver une fiche avant de la modifier.
 */
export default function MemberFloraManager() {
  const qc = useQueryClient();
  const toast = useToast();
  const [search, setSearch] = useState('');
  const { data, isLoading } = useQuery({ queryKey: ['flora', 'member', search], queryFn: () => floraApi.list({ limit: 100, q: search }) });
  ...
  const startEdit = async (f) => {
    const full = (await floraApi.get(f.id)).data;
    setEditingId(full.id);
    const extra = {};
    Object.keys(FLORA_EXTRA_EMPTY).forEach((k) => { extra[k] = full[k] ?? ''; });
    setForm({
      ...
      description: full.description ?? '', habitat: full.habitat ?? '', characteristics: full.characteristics ?? '',
      uses: full.uses ?? '', status: full.status ?? 'published',
      ...extra,
    });
  ...
  const removeImageMutation = useMutation({
    mutationFn: (imageId) => floraApi.removeImage(imageId),
    onSuccess: (_, imageId) => {
      setExistingImages((imgs) => imgs.filter((i) => i.id !== imageId));
      qc.invalidateQueries({ queryKey: ['flora'] });
  ...
  const saveMutation = useMutation({
    mutationFn: () => {
      ...
      return editingId ? floraApi.update(editingId, fd) : floraApi.create(fd);
    },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['flora'] });
  ...
  const removeMutation = useMutation({
    mutationFn: (id) => floraApi.remove(id),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['flora'] });
      qc.invalidateQueries({ queryKey: ['stats'] });
  ...
  return (
    <div>
      <h1 className="font-display text-2xl font-medium text-ink">🌿 Flore — Gestion</h1>
      ...
      <form onSubmit={(e) => { e.preventDefault(); saveMutation.mutate(); }} className="mt-6 grid gap-3 rounded-[2rem] border border-earth-100 bg-white p-6 sm:grid-cols-2">
        ...
        <SpeciesFormSections sections={[FLORA_SECTIONS[0]]} form={form} set={set} />
        ...
        <textarea placeholder="Caractéristiques" className="input" value={form.characteristics} onChange={set('characteristics')} />
        <textarea placeholder="Utilisation" className="input" value={form.uses} onChange={set('uses')} />

        <SpeciesFormSections sections={FLORA_SECTIONS.slice(1, 8)} form={form} set={set} />
        ...
        <SpeciesFormSections sections={[FLORA_SECTIONS[8]]} form={form} set={set} />

        <SpeciesFormSections sections={FLORA_SECTIONS.slice(9)} form={form} set={set} />
